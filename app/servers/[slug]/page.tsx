      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap mb-1">
                <Link
                  ...
                  className="text-xs bg-[#1e1e2e] text-[#8888aa] px-2 py-0.5 rounded-full hover:text-white transition-colors"
                >
                  {cat.label}
                </Link>
                {server.verified && (
              ...
              <h1 className="text-2xl sm:text-3xl font-bold text-white">{server.name}</h1>
              <p className="text-[#8888aa] mt-1">by {server.author}</p>
            </div>
          </div>

          <p className="text-lg text-[#ccccdd] leading-relaxed mb-8">{server.description}</p>

          <div className="bg-[#12121a] border border-[#1e1e2e] rounded-xl p-6 mb-6">
            <h2 className="font-semibold text-white mb-4">About</h2>
            <p className="text-[#8888aa] leading-relaxed">{server.longDescription}</p>
          </div>
            ...
            <div className="mb-6">
              ...
              <div className="flex flex-wrap gap-2">
                {server.tags.map((tag) => (
                  <span
                    key={tag}
                    className="text-xs bg-[#12121a] border border-[#1e1e2e] text-[#8888aa] px-3 py-1 rounded-full"
                  >
                    #{tag}
                  </span>
                ))}
        ...
        <div className="space-y-4">
          ...
          <div className="bg-[#12121a] border border-[#1e1e2e] rounded-xl p-5 space-y-3">
            ...
            <div className="flex justify-between items-center">
              <span className="text-[#8888aa] text-sm">Language</span>
              <span className="text-sm text-white">{server.language}</span>
            </div>
            {server.version && (
              <div className="flex justify-between items-center">
                <span className="text-[#8888aa] text-sm">Version</span>
                <span className="text-sm text-white">v{server.version}</span>
              </div>
            )}
            <div className="flex justify-between items-center">
